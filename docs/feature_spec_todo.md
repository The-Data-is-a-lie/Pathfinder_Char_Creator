...
**Status — buckets ready (2026-07):** `spell_buffs.json` (regenerated by
`Backend/scripts/build_spell_buffs.py`) now classifies every buff spell with the same sentence
classifier as items (`build_item_changes.py`), into the four item-style buckets per spell:
`changes` (unconditional numbers) · `contextNotes` targeting skills · `contextNotes` targeting
other rolls · `unplaced` (effect text with no valid anchor; those spells get **no** Buffs-tab
buff). The buckets are the reference data for the conditionals work below.

**To do (in order):**
- [ ] **Mine the buckets for conditional candidates.** `contextNotes` entries whose target is
  `attack`/`cmb`/`damage`-adjacent are situational combat bonuses — per the OKF
  `pathfinder-system/conditionals` decision rules those should become **default-off conditionals
  on the main weapon's attack action**, not stay advisory text. The `unplaced` bucket is the
  unstructured pool to mine for riders (saves, conditions, ability damage).
- [ ] **Draft → curate pipeline.** `Backend/scripts/build_spell_conditionals.py` already drafts
  `Backend/json/spells/spell_conditionals.draft.json` from `data/spells.csv` (bucket A =
  attack/damage buffs, bucket B = touch-attack riders). Hand-curate the trusted entries into
  `Backend/json/spells/spell_changes.json` (A) and `Backend/json/spells/spell_riders.json` (B);
  strip `review`/`_*` keys, finalize CL scaling/caps.
- [ ] **Module attachment.** Attach curated spell conditionals the same way talent/maneuver
  conditionals attach (main weapon's attack action; house `[[ ]]` rider style per
  `.claude/skills/foundry-conditionals`). `spell_riders.json` is not consumed anywhere yet.
...
**Your spec:** which spells first (the NPC's own castable buffs vs everything in
`spell_buffs.json`), and whether riders should also fire from the spell item itself when the
caster attacks with it.
