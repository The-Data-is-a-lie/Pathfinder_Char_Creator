...
def randomize_level(character, min_num, max_num, len_flaw=0):
    ...
    if character.c_class_2 == '':
        ...
        c_class_level = level
        c_class_2_level = 0
        update_level(character, level, c_class_level, c_class_2_level, len_flaw)
...
def update_level(character, level, c_class_level, c_class_2_level, len_flaw=None, homebrew_amount=None):
    ...
    if len_flaw != 0:
        character.normal_feat_amount = ceil(character.level/2)
        character.flaw_feat_amount = min(max(floor(len(character.flaw)/2 + 1),1), 3) #So it's always 1<feats<3
