...
def process_input_values(input_values):
    try:
        ...
        # Unpack input_values
        create_new_char, userInput_region, userInput_race, class_choice, multi_class, alignment_input, deity, userInput_gender, truly_random_feats, inherents, homebrew_feat_amount, num_dice, num_sides, high_level, low_level, gold_num = input_values
        session['character_data'] = generate_random_char(create_new_char, userInput_region, userInput_race, class_choice, multi_class, alignment_input, deity, inherents, homebrew_feat_amount, userInput_gender, truly_random_feats, num_dice, num_sides, high_level, low_level, gold_num)
...
def update_character_data():
    ...
    for key, value in data.items():
        if key in ('input2', 'input12', 'input13', 'input14', 'input15', 'input16'):
            value = int(value)
        else:
            value = value.strip()
