...
def phase_gear_and_equipment(character):
	...
	# The V4 wall-pass patch lived here and is gone. It forced a shield on for roles that declared
	# one because shield_chooser could not deliver: it only returned on its ~10% Tower branch, and
	# shield_flag_func mutated then returned None, so no character had ever worn one. Both are
	# fixed, and the role's promise now lives inside shield_chooser next to the roll it overrides.
	limits = shield_chooser(character, character.weapon_dict)
	character.shield_flag = shield_flag_func(character, limits=limits)
	character.shield_dict = list_selection(character, 'armor', limits=limits,
										   shield_flag=character.shield_flag,
										   allow=character.shield_allow)
...
def generate_random_char(create_new_char='Y', userInput_region="Tal-Falko", userInput_race='Orc', class_choice='wizard', chosen_BAB='low', chosen_caster_level = 'random', multi_class='N', 
	...
	# ------------------- Backstory (coherent prose via Ollama, template fallback) -------------------#
		...
		payload.update({
				...
				# Sheet issue #69: flat { "<enhancement, lowercase>": "<rules text>" } for the
				# gear's chosen enhancements. The web sheet reads this as its TOP-priority
				# enhancement-description source (compendium fallback today); the FoundryVTT
				# module can consume the same field. Appended after `mythic` (the current tail) so
				# no existing content key shifts position (validate_luck's tail check names it).
				"enhancement_desc_dict": enhancement_desc_dict,
				})
		...
		for _section, _chosen, _qe_section, _desc_map in (
			...
			for _q_name in dict.fromkeys(_chosen or []):
				...
				# The flat desc dict ships every chosen quality's text, scraped list first,
				# curated entry as fallback. Plain "+N" tiers have no text here on purpose:
				# the sheet words numeric bonuses itself.
				_flat_desc = _q_desc or str(_q_entry.get("description") or "")
				if _flat_desc:
					enhancement_desc_dict.setdefault(str(_q_name).lower(), _flat_desc)
